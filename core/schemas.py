...
class CostSchema(BaseModel):
    ...
    amount: float = Field(
        ...,
        gt=0,
        description="The amount of the cost item(most be over greater than zero)",
    )
...
class UserSchema(BaseModel):
    user_name: str = Field(
        ..., min_length=1, max_length=150, description="The username"
    )
    password: str = Field(
        ..., min_length=1, max_length=150, description="The user password"
    )
